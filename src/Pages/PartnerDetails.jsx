...
const PartnerDetails = () => {
  ...
  if (!data || !data._id) {
    return (
      <p className="text-center text-4xl my-20 text-gray-500">Loading...</p>
    );
  }
    ...
    <div className="flex flex-col md:flex-row gap-8 px-10 py-20 items-center">
      ...
      <div className="flex-1 space-y-2">
        ...
        <p className="font-semibold text-[14px]">
          <span className="font-bold text-lg text-[#4F959D]">{subject}</span>
        </p>
        ...
        <p className="text-xl text-gray-500 my-5">
          <span className="font-semibold">Contact :</span> {location} - {email}
        </p>
        ...
        <div className="flex gap-3">
          <button
            ...
            disabled={install}
            className="btn bg-[#4F959D] disabled:bg-gray-500 disabled:cursor-not-allowed inline-block text-white font-semibold px-5 rounded-md"
          >
            {install ? "Already Added" : "Add Partner"}
          </button>
          <button
            onClick={() => navigate(-1)}
            className="btn bg-[#4F959D] inline-block text-white font-semibold px-5 rounded-md"
          >
            Go Back
          </button>
        </div>
